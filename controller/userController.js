  ...
  try {
    ...
    if (!isMatch) {
      return res.status(400).json({
        success: false,
        message: "Currentpassword do not match the password in the database",
      });
    }
...
const loadShop = async (req, res) => {
  try {
    ...
    let productData = await productSchema
      .find(filter)
      .skip(skip)
...
const addToCart = async (req, res) => {
  ...
  try {
    ...
    if (quantity > 10) {
      return res.status(400).json({
        success: false,
        message: "You can only add a maximum of 10 items",
      });
    }
    ...
    if (existingProductIndex >= 0) {
      ...
      if (totalQuantity > 10) {
        return res.status(400).json({
          success: false,
          message:
            "You can only have a maximum of 10 items of the same size in the cart",
        });
      }
  ...
  try {
    ...
    if (!variant) {
      return res.status(404).json({
        success: false,
        message: "Variant not found for selected size",
      });
    }
    ...
    if (existingProductIndex >= 0) {
      return res.status(400).json({
        success: false,
        message: "Product with same size already exist in cart",
      });
    } else {
      cart.items.push({ productId, quantity, size });
    }
...
const placeOrder = async (req, res) => {
  try {
    let { paymentMethod, userId, addressId, items, totalAmount, couponCode } =
      req.body;
    ...
    if (paymentMethod !== "COD") {
      return res.status(400).json({
        success: false,
        message: "Only COD payment method is allowed",
      });
    }
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      if (!variant) {
        return res.status(404).json({
          success: false,
          message: `Variant of size ${items[i].size} not found`,
        });
      }
...
const razorPayOrder = async (req, res) => {
  try {
    const { addressId, userId, items, totalAmount, paymentMethod, couponCode } =
      req.body;
...
const retryPayment = async (req, res) => {
  ...
  try {
    ...
    if (!order.razorpayOrderId) {
      res.status(404).json({
        success: false,
        message: "No orders found with the razorpay order ID",
      });
    }
    res.status(200).json({
      success: true,
    ...
    res.status(500).json({
      success: false,
      message: "Failed to fetch retry payment details",
    });
  }
};
...
const requestItemReturn = async (req, res) => {
  try {
    ...
    if (item.itemStatus === "Requested") {
      return res.status(400).json({
        success: false,
        message: "Return request already made for this item",
      });
    }
  ...
  try {
    ...
    if (!wishlist) {
      wishlist = new wishlistSchema({
        userId,
        products: [{ productId, size: numericalSize }],
      });
    } else {
...
const downloadReceipt = async (req, res) => {
  try {
    ...
    const order = await orderSchema
      ...
      .populate("orderItems.productId", "name offerPrice");
